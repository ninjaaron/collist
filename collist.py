...
def representation(iterable):
    ...
    if isinstance(iterable, dict):
        strlist = [
            u"{}: {}".format(repr(k), repr(v)) + u"," for k, v in iterable.items()
        ]
        divchar = u"{}"
    else:
    ...
    table = []
    for row in rows:
        row = u"".join([u" {0:{1}}".format(i, tabs) for i in row]).lstrip()
        table.append(row)
...
def main():
    ...
    prs.add_argument("-d", "--divider", default="  ", help="defaults to two spaces")
